...
var Service = {
    list: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.Evaluation.scope('nested').findAndCount(
    ...
    insert: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.Evaluation.create(params);
        }).then(function(row) {
    ...
    update: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.Evaluation.scope('nested').findOne({
                where: {
                    id: params.id
                }
            });
            ...
            if (!row) {
                throw errors.types.invalidParams({
                    path: 'id', message: 'Evaluation with the specified id cannot be found',
                });
            }
    ...
    filters: function(params, callback, sid, req) {
        session.verify(req).then(function() {
            return helpers.fetchFilters(params, models.Evaluation);
