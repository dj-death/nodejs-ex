...
var Service = {
    list: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.KPI.scope('nested').findAndCount(
                helpers.sequelizify(params, models.KPI));
    ...
    insert: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.KPI.create(params);
        }).then(function(row) {
    ...
    update: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.KPI.scope('nested').findOne({
                where: {
                    id: params.id
                }
            });
    ...
    remove: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            if (!row) {
                throw errors.types.invalidParams({
                    path: 'id', message: 'KPI with the specified id cannot be found',
                });
            }
            return row.destroy();
        }).then(function(row) {
    ...
    filters: function(params, callback, sid, req) {
        session.verify(req).then(function() {
            return helpers.fetchFilters(params, models.KPI);
