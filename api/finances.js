...
var Service = {
    list: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.Finance.scope('nested').findAndCount(
                helpers.sequelizify(params, models.Finance));
    ...
    insert: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.Finance.create(params);
        }).then(function(row) {
    ...
    update: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.Finance.scope('nested').findOne({
                where: {
                    id: params.id
                }
            });
    ...
    remove: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            if (!row) {
                throw errors.types.invalidParams({
                    path: 'id', message: 'Finance with the specified id cannot be found',
                });
            }
            return row.destroy();
        }).then(function(row) {
    ...
    filters: function(params, callback, sid, req) {
        session.verify(req).then(function() {
            return helpers.fetchFilters(params, models.Finance);
