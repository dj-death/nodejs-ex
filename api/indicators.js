...
var Service = {
    list: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.Indicator.scope('nested').findAndCount(
                helpers.sequelizify(params, models.Indicator));
    ...
    insert: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.Indicator.create(params);
        }).then(function(row) {
    ...
    update: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.Indicator.scope('nested').findOne({
                where: {
                    id: params.id
                }
            });
    ...
    remove: function(params, callback, sid, req) {
        session.verify(req).then(function(session) {
            ...
            return models.Indicator.findOne({
                where: {
                    id: params.id
                }
            });
            ...
            if (!row) {
                throw errors.types.invalidParams({
                    path: 'id', message: 'Indicator with the specified id cannot be found',
                });
            }
            return row.destroy();
        }).then(function(row) {
    ...
    filters: function(params, callback, sid, req) {
        session.verify(req).then(function() {
            return helpers.fetchFilters(params, models.Indicator);
